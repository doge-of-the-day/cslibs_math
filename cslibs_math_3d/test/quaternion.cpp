...
TEST(Test_cslibs_math_3d, testYawConstructor) {
  ...
  for (std::size_t i = 0; i < ITERATIONS; ++i) {
    ...
    EXPECT_NEAR(tf_q.z(), q.z(), 1e-6);
    EXPECT_TRUE(
        cslibs_math::common::eq(yaw, q.yaw(), 1e-6) ||
  ...
  cslibs_math::utility::tiny_time::time_t start =
      cslibs_math::utility::tiny_time::clock_t::now();
  for (std::size_t i = 0; i < ITERATIONS; ++i) {
  ...
  std::cout << "[runtimes  ]"
            << "\n";
  std::cout << "[cslibs    ]: "
            << cslibs_math::utility::tiny_time::microseconds(dur) /
                   static_cast<double>(ITERATIONS)
            << "µs \n";
  std::cout << "[tf        ]: "
            << cslibs_math::utility::tiny_time::microseconds(dur_tf) /
                   static_cast<double>(ITERATIONS)
            << "µs \n";
}
...
TEST(Test_cslibs_math_3d, testRollPitchYawConstructor) {
  ...
  for (std::size_t i = 0; i < ITERATIONS; ++i) {
    ...
    EXPECT_TRUE(cslibs_math::common::eq(roll, q.roll(), 1e-6) ||
                cslibs_math::common::eq(std::abs(roll) + std::abs(q.roll()),
                                        M_PI, 1e-6));
    EXPECT_TRUE(
        cslibs_math::common::eq(pitch, q.pitch(), 1e-6) ||
...
TEST(Test_cslibs_math_3d, testCopyMoveConstructor) {
  ...
  for (std::size_t i = 0; i < ITERATIONS; ++i) {
    const auto q00 = q0;
    const cslibs_math_3d::Quaterniond q1(std::move(q00));
    EXPECT_EQ(q0.x(), q1.x());
    EXPECT_EQ(q0.y(), q1.y());
    EXPECT_EQ(q0.z(), q1.z());
    EXPECT_EQ(q0.w(), q1.w());
  ...
  std::cout << "[runtimes  ]"
            << "\n";
  std::cout << "[copy      ]: "
            << cslibs_math::utility::tiny_time::microseconds(dur_copy) /
                   static_cast<double>(ITERATIONS)
            << "µs \n";
  std::cout << "[move      ]: "
            << cslibs_math::utility::tiny_time::microseconds(dur_move) /
                   static_cast<double>(ITERATIONS)
            << "µs \n";
}
...
TEST(Test_cslibs_math_3d, testOperatorPlus) {
  ...
  std::cout << "[tf        ]: "
            ...
            << "µs \n";
  std::cout << "[cslibs    ]: "
            << cslibs_math::utility::tiny_time::microseconds(dur_assign) /
                   static_cast<double>(ITERATIONS)
            << "µs \n";
  std::cout << "[tf        ]: "
            << cslibs_math::utility::tiny_time::microseconds(dur_tf_assign) /
                   static_cast<double>(ITERATIONS)
            << "µs \n";
}
