...
class EIGEN_ALIGN16 NDTIterator {
 public:
  ...
  inline explicit NDTIterator(const index_t &start, const index_t &end)
      : NDTIterator{point_t{start[0], start[1], start[2]},
                    point_t{end[0], end[1], end[2]}, 1.0} {}

  inline explicit NDTIterator(const point_t &p0, const point_t &p1,
                              const T &resolution)
    ...
    for (std::size_t i = 0; i < 3; ++i) minmax(i);
  ...
  inline bool done() const {
    return (iteration_ < 0) || (index_[0] < min_[0]) || (index_[0] > max_[0]) ||
           (index_[1] < min_[1]) || (index_[1] > max_[1]) ||
           (index_[2] < min_[2]) || (index_[2] > max_[2]);
  }
