...
class EIGEN_ALIGN16 Transform3 {
 public:
  ...
  inline Transform3(const Transform3 &other) = default;
  inline Transform3(Transform3 &&other) = default;

  inline explicit Transform3(const T x, const T y, const T z)
      : translation_{x, y, z} {}

  inline explicit Transform3(const translation_t &translation)
      : translation_{translation} {}

  inline explicit Transform3(const T yaw) : rotation_{yaw} {}

  inline explicit Transform3(const T x, const T y, const T z, const T yaw)
      : translation_{x, y, z}, rotation_{yaw} {}

  inline explicit Transform3(const translation_t &translation, const T yaw)
      : translation_{translation}, rotation_{yaw} {}

  inline explicit Transform3(const translation_t &translation,
                             const rotation_t &rotation)
      : translation_{translation}, rotation_{rotation} {}

  inline explicit Transform3(const T x, const T y, const T z, const T roll,
                             const T pitch, const T yaw)
  ...
  inline Transform3 inverse() const {
    ...
    return Transform3(rotation_inverse * -translation_, rotation_inverse);
  }

  inline Transform3 operator-() const { return inverse(); }

  inline T &tx() { return translation_(0); }

  inline T tx() const { return translation_(0); }

  inline T &ty() { return translation_(1); }

  inline T ty() const { return translation_(1); }

  inline T &tz() { return translation_(2); }

  inline T tz() const { return translation_(2); }

  inline T roll() const { return rotation_.roll(); }

  inline T pitch() const { return rotation_.pitch(); }

  inline T yaw() const { return rotation_.yaw(); }

  inline void setRoll(const T roll) { rotation_.setRoll(roll); }

  inline void setPitch(const T pitch) { rotation_.setPitch(pitch); }

  inline void setYaw(const T yaw) { rotation_.setYaw(yaw); }

  inline void setRPY(const T roll, const T pitch, const T yaw) {
    rotation_.setRPY(roll, pitch, yaw);
  }

  inline translation_t &translation() { return translation_; }

  inline translation_t const &translation() const { return translation_; }

  inline rotation_t &rotation() { return rotation_; }

  inline rotation_t const &rotation() const { return rotation_; }

  inline eigen_vector_3d_t const euler() const {
    return cslibs_math::linear::eigen::create<eigen_vector_3d_t>(
        rotation_.roll(), rotation_.pitch(), rotation_.yaw());
  }

  inline eigen_vector_6d_t toEigen() const {
    return cslibs_math::linear::eigen::create<eigen_vector_6d_t>(
        translation_(0), translation_(1), translation_(2), rotation_.roll(),
  ...
  inline void setFrom(const T x, const T y, const T z, const T roll,
                      const T pitch, const T yaw) {
    translation_(0) = x;
    translation_(1) = y;
    translation_(2) = z;
  ...
  inline Transform3 interpolate(const Transform3 &other, const T ratio) const {
    assert(ratio >= 0.0);
    assert(ratio <= 1.0);
    if (ratio == T()) {
      return *this;
    ...
    const translation_t translation =
        translation_ * ratio_inverse + other.translation_ * ratio;
...
inline Eigen::Matrix<T, 3, 1> operator*(const cslibs_math_3d::Transform3<T> &t,
  ...
  return (t.rotation() * cslibs_math_3d::Vector3<T>(v) + t.translation())
      .data();
}

template <typename T>
inline cslibs_math_3d::Transform3<T> operator*(
    const cslibs_math_3d::Transform3<T> &a,
    const cslibs_math_3d::Transform3<T> &b) {
  return cslibs_math_3d::Transform3<T>(
      a.translation() + a.rotation() * b.translation(),
      a.rotation() * b.rotation());
}

template <typename T>
inline cslibs_math::linear::Vector<T, 2> operator*(
    const cslibs_math_3d::Transform3<T> &t,
...
inline std::string to_string(const cslibs_math_3d::Transform3<T> &t) {
  return "[" + std::to_string(t.tx()) + " " + std::to_string(t.ty()) + " " +
         std::to_string(t.tz()) + " " + std::to_string(t.roll()) + " " +
         std::to_string(t.pitch()) + " " + std::to_string(t.yaw()) + "]";
}
}  // namespace std
