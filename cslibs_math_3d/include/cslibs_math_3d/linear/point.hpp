...
class EIGEN_ALIGN16 PointRGB3 {
 public:
  ...
  inline PointRGB3(const point_t &pos) : point_{pos} {}

  inline PointRGB3(const T &v) : point_{v} {}

  inline PointRGB3(const point_t &pos, const T &a, const color_t &c)
      : point_{pos}, a_{a}, color_{c} {}

  virtual ~PointRGB3() = default;

  inline point_t getPoint() const { return point_; }

  inline Tc getAlpha() const { return a_; }

  inline color_t getColor() const { return color_; }

  inline void setPoint(const point_t &point) { point_ = point; }

  inline void setAlpha(const Tc a) { a_ = a; }

  inline void getColor(const color_t &c) { color_ = c; }
  ...
  inline static PointRGB3 inf() { return PointRGB3{point_t::inf()}; }

  inline static PointRGB3 max() { return PointRGB3{point_t::max()}; }

  inline static PointRGB3 min() { return PointRGB3{point_t::min()}; }
