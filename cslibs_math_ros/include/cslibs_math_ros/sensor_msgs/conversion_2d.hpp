...
inline cslibs_time::TimeFrame from(
    const ::sensor_msgs::LaserScan::ConstPtr &src) {
  const ros::Time start_stamp = src->header.stamp;
  ros::Duration delta_stamp =
      ros::Duration(static_cast<double>(src->time_increment)) *
      static_cast<double>(src->ranges.size());
  if (delta_stamp <= ros::Duration(0.0)) {
    delta_stamp = ros::Duration(static_cast<double>(src->scan_time));
  }
  return cslibs_time::TimeFrame(
      start_stamp.toNSec(),
...
inline cslibs_time::TimeFrame from(
  ...
  const double start_off_ratio =
      static_cast<double>(std::max(src->angle_min, angular_interval[0]) -
                          src->angle_min) /
      fov;
  const double end_off_ratio =
      static_cast<double>(std::min(src->angle_max, angular_interval[1]) -
                          src->angle_min) /
      fov;

  const int64_t start_time = static_cast<int64_t>(std::floor(
      start_stamp.toNSec() + delta_stamp.toNSec() * start_off_ratio + 0.5));
  const int64_t end_time = static_cast<int64_t>(std::floor(
      start_stamp.toNSec() + delta_stamp.toNSec() * end_off_ratio + 0.5));
...
inline void from(const ::sensor_msgs::LaserScan::ConstPtr &src,
                 typename cslibs_math_2d::PolarPointcloud2<T>::Ptr &dst) {
  from(src, {{src->range_min, src->range_max}},
       {{src->angle_min, src->angle_max}}, dst);
}
  ...
  for (const float range : src->ranges) {
    if (in_linear_interval(range) && in_angular_interval(angle)) {
      const cslibs_math_2d::Point2<T> p(
          static_cast<T>(std::cos(angle) * range),
...
inline void from(const ::sensor_msgs::LaserScan::ConstPtr &src,
  ...
  ros::Duration delta_stamp =
      ros::Duration(static_cast<double>(src->time_increment));
  if (delta_stamp <= ros::Duration(0.0)) {
    delta_stamp = ros::Duration(static_cast<double>(
        src->scan_time / static_cast<float>(src->ranges.size())));
  }
  ...
  if (!tfl.lookupTransform(fixed_frame, src->header.frame_id, end_stamp,
                           f_T_end, tf_timeout)) {
    return;
  }
  if (!tfl.lookupTransform(fixed_frame, src->header.frame_id, start_stamp,
                           f_T_start, tf_timeout)) {
    return;
  }
  ...
  for (const auto range : src->ranges) {
    if (in_linear_interval(range) && in_angular_interval(angle)) {
      cslibs_math_2d::Transform2<T> Tf =
          cslibs_math_2d::Transform2<T>::identity().interpolate(end_T_start,
                                                                dt);
      cslibs_math_2d::PolarPoint2<T> pt =
          Tf * cslibs_math_2d::Point2<T>(std::cos(angle) * range,
                                         std::sin(angle) * range);
      dst->insert(pt);
    } else {
      dst->insertInvalid();
...
inline void from(const ::sensor_msgs::LaserScan::ConstPtr &src,
                 ...
                 typename cslibs_math_2d::PolarPointcloud2<T>::Ptr &dst) {
  from(src, {{src->range_min, src->range_max}},
       {{src->angle_min, src->angle_max}}, fixed_frame, tf_timeout, tfl, dst);
}
}  // namespace conversion_2d
}  // namespace sensor_msgs
}  // namespace cslibs_math_ros
