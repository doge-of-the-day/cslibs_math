...
TEST(Test_cslibs_math, testDistributionCovariance) {
  ...
  EXPECT_NEAR(test_distribution_200.covariance(0, 0), stable_cov(0, 0),
              tolerance);
  EXPECT_NEAR(test_distribution_200.covariance(0, 1), stable_cov(0, 1),
              tolerance);
  EXPECT_NEAR(test_distribution_200.covariance(1, 0), stable_cov(1, 0),
              tolerance);
  EXPECT_NEAR(test_distribution_200.covariance(1, 1), stable_cov(1, 1),
              tolerance);
  ...
  EXPECT_NEAR(test_distribution_500.covariance(0, 0), stable_cov(0, 0),
              tolerance);
  EXPECT_NEAR(test_distribution_500.covariance(0, 1), stable_cov(0, 1),
              tolerance);
  EXPECT_NEAR(test_distribution_500.covariance(1, 0), stable_cov(1, 0),
              tolerance);
  EXPECT_NEAR(test_distribution_500.covariance(1, 1), stable_cov(1, 1),
              tolerance);
  ...
  EXPECT_NEAR(test_distribution_5000.covariance(0, 0), stable_cov(0, 0),
              tolerance);
  EXPECT_NEAR(test_distribution_5000.covariance(0, 1), stable_cov(0, 1),
              tolerance);
  EXPECT_NEAR(test_distribution_5000.covariance(1, 0), stable_cov(1, 0),
              tolerance);
  EXPECT_NEAR(test_distribution_5000.covariance(1, 1), stable_cov(1, 1),
              tolerance);
}
...
TEST(Test_cslibs_math, testDistributionEigenValues) {
  ...
  EXPECT_NEAR(test_distribution_200.eigen_values(0), eigen_values(0),
              tolerance);
  EXPECT_NEAR(test_distribution_200.eigen_values(1), eigen_values(1),
              tolerance);
  ...
  eigen_values = distribution.getEigenValues();
  EXPECT_NEAR(test_distribution_500.eigen_values(0), eigen_values(1),
              tolerance);
  EXPECT_NEAR(test_distribution_500.eigen_values(1), eigen_values(0),
              tolerance);
  ...
  eigen_values = distribution.getEigenValues();
  EXPECT_NEAR(test_distribution_5000.eigen_values(0), eigen_values(0),
              tolerance);
  EXPECT_NEAR(test_distribution_5000.eigen_values(1), eigen_values(1),
              tolerance);
}

TEST(Test_cslibs_math, testDistributionEigenVectors) {
  auto equals = [](const Eigen::Vector2d &a, const Eigen::Vector2d &b,
                   const double eps) {
  ...
  bool condition =
      (equals(exp_a, rec_a, tolerance) && equals(exp_b, rec_b, tolerance)) ||
...
TEST(Test_cslibs_math, testDistributionCopy) {
  ...
  EXPECT_TRUE(distribution_a.getInformationMatrix() ==
              distribution_b.getInformationMatrix());

  EXPECT_TRUE(distribution_a.getEigenValues() ==
              distribution_b.getEigenValues());
  EXPECT_TRUE(distribution_a.getEigenVectors() ==
              distribution_b.getEigenVectors());
}
...
TEST(Test_cslibs_math, testDistributionAddition) {
  ...
  EXPECT_NEAR((distribution_a.getMean() - distribution_b.getMean()).norm(), 0.0,
              tolerance);
  EXPECT_TRUE(distribution_a.getN() == distribution_b.getN());

  EXPECT_NEAR(
      (distribution_a.getCovariance() - distribution_b.getCovariance()).norm(),
      0.0, tolerance);
  EXPECT_NEAR((distribution_a.getInformationMatrix() -
               distribution_b.getInformationMatrix())
                  .norm(),
              0.0, tolerance);

  EXPECT_NEAR(
      (distribution_a.getEigenValues() - distribution_b.getEigenValues())
          .norm(),
      0.0, tolerance);
  EXPECT_NEAR(
      (distribution_a.getEigenVectors() - distribution_b.getEigenVectors())
          .norm(),
      0.0, tolerance);
...
TEST(Test_cslibs_math, testWeightedDistributionAddition) {
  testWeightedDistributionAddition<cs::WeightedDistribution<double, 2>>(
      test_distribution_200);
  testWeightedDistributionAddition<cs::WeightedDistribution<double, 2>>(
      test_distribution_500);
  testWeightedDistributionAddition<cs::WeightedDistribution<double, 2>>(
      test_distribution_5000);
  testWeightedDistributionAddition<cs::StableWeightedDistribution<double, 2>>(
      test_distribution_200);
  testWeightedDistributionAddition<cs::StableWeightedDistribution<double, 2>>(
      test_distribution_500);
  testWeightedDistributionAddition<cs::StableWeightedDistribution<double, 2>>(
  ...
  testWeightedDistributionScale<cs::WeightedDistribution<double, 2>>(
      test_distribution_200);
  testWeightedDistributionScale<cs::WeightedDistribution<double, 2>>(
      test_distribution_500);
  testWeightedDistributionScale<cs::WeightedDistribution<double, 2>>(
      test_distribution_5000);
  testWeightedDistributionScale<cs::StableWeightedDistribution<double, 2>>(
      test_distribution_200);
  testWeightedDistributionScale<cs::StableWeightedDistribution<double, 2>>(
      test_distribution_500);
  testWeightedDistributionScale<cs::StableWeightedDistribution<double, 2>>(
...
int main(int argc, char *argv[]) {
  ...
  if (!nh_private.getParam("test_distribution_200",
                           test_distribution_200_path)) {
    std::cerr << "[TestNodeDistribution]: Cannot load test_distribution_200!"
              << std::endl;
    std::cout << test_distribution_200_path << std::endl;
    return -1;
  }
  if (!nh_private.getParam("test_distribution_500",
                           test_distribution_500_path)) {
    std::cerr << "[TestNodeDistribution]: Cannot load test_distribution_500!"
              << std::endl;
    return -1;
  }
  if (!nh_private.getParam("test_distribution_5000",
                           test_distribution_5000_path)) {
    std::cerr << "[TestNodeDistribution]: Cannot load test_distribution_5000!"
              << std::endl;
    return -1;
  }
