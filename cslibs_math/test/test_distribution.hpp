...
struct EIGEN_ALIGN16 TestDistribution {
  ...
  template <std::size_t rows, std::size_t cols>
  void read(const YAML::Node &yaml, Eigen::Matrix<double, rows, cols> &mat) {
  ...
  void read(
      ...
      std::vector<Eigen::Matrix<double, rows, cols>,
                  Eigen::aligned_allocator<Eigen::Matrix<double, rows, cols>>>
          &mats) {
