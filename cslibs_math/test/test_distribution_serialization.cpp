...
struct TestDimension {
  void operator()() {
    ...
    using distribution_t =
        cslibs_math::statistics::Distribution<double, Dim, 0>;
    for (std::size_t i = 0; i < REPETITIONS; ++i) {
      distribution_t d;

      for (std::size_t n = 0;
           n < static_cast<std::size_t>(rng_num_samples.get()); ++n)
        d.add(rng.get());
      ...
      for (std::size_t i = 0; i < Dim; ++i) {
        ...
        for (std::size_t j = 0; j < Dim; ++j) {
          EXPECT_NEAR(d.getCorrelated()(i, j),
                      d_converted.getCorrelated()(i, j), 1e-6);
          EXPECT_NEAR(d.getCovariance()(i, j),
                      d_converted.getCovariance()(i, j), 1e-6);
          EXPECT_NEAR(d.getInformationMatrix()(i, j),
                      d_converted.getInformationMatrix()(i, j), 1e-6);
        }
      }
...
struct TestDimension<1> {
  void operator()() {
    ...
    for (std::size_t i = 0; i < REPETITIONS; ++i) {
      ...
      EXPECT_NEAR(d.getStandardDeviation(), d_converted.getStandardDeviation(),
                  1e-6);
    }
  }
};
