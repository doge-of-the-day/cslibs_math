...
static void applicationFractional(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    if (cslibs_math::common::eq(1.0, res)) {
      std::cerr << "There went something wrong in applicationFractional "
...
static void applicationLog(benchmark::State& state) {
  auto task = [&state]() {
    using Log2d = cslibs_math::approx::Log<double>;
  ...
  for (auto _ : state) {
    const auto res = task();
    if (cslibs_math::common::eq(1.0, res)) {
      std::cerr << "There went something wrong in applicationLog "
                << std::setprecision(20) << res << std::endl;
    }
  }
}

static void applicationPlainLog(benchmark::State& state) {
  auto task = [&state]() {
    cslibs_math::random::Uniform<double, 1> rng(0.0, 1.0);
    cslibs_math::utility::tiny_time::duration_t elapsed;
