...
struct binary<cslibs_math::statistics::StableWeightedDistribution, T, Dim,
              lambda_ratio_exponent> {
  using distribution_t = cslibs_math::statistics::StableWeightedDistribution<
      T, Dim, lambda_ratio_exponent>;
  using sample_t = typename distribution_t::sample_t;
  using correlated_t = typename distribution_t::covariance_t;

  static const std::size_t size = sizeof(std::size_t) + 2 * sizeof(T) +
                                  Dim * sizeof(T) + Dim * Dim * sizeof(T);
...
namespace YAML {
template <typename T, std::size_t Dim, std::size_t lambda_ratio_exponent>
struct convert<cslibs_math::statistics::StableWeightedDistribution<
    T, Dim, lambda_ratio_exponent>> {
  using sample_t = typename cslibs_math::statistics::StableWeightedDistribution<
      T, Dim, lambda_ratio_exponent>::sample_t;
  using covariance_t =
      typename cslibs_math::statistics::StableWeightedDistribution<
          T, Dim, lambda_ratio_exponent>::covariance_t;

  static Node encode(const cslibs_math::statistics::StableWeightedDistribution<
    ...
    for (std::size_t i = 0; i < Dim; ++i)
      for (std::size_t j = 0; j < Dim; ++j) n.push_back(s(i, j));

    return n;
  }

  static bool decode(const Node &n,
                     cslibs_math::statistics::StableWeightedDistribution<
                         T, Dim, lambda_ratio_exponent> &rhs) {
    if (!n.IsSequence() || n.size() != (3 + Dim + Dim * Dim)) return false;
    ...
    for (std::size_t i = 0; i < Dim; ++i) mean(i) = n[p++].as<T>();

    covariance_t correlated(covariance_t::Zero());
    for (std::size_t i = 0; i < Dim; ++i)
      for (std::size_t j = 0; j < Dim; ++j) correlated(i, j) = n[p++].as<T>();

    rhs = cslibs_math::statistics::StableWeightedDistribution<
        T, Dim, lambda_ratio_exponent>(sc, w, w_sq, mean, correlated);
    return true;
  }
};
...
struct convert<cslibs_math::statistics::StableWeightedDistribution<
    T, 1, lambda_ratio_exponent>> {
  static Node encode(const cslibs_math::statistics::StableWeightedDistribution<
                     T, 1, lambda_ratio_exponent> &rhs) {
    Node n;
    n.push_back(rhs.getSampleCount());
    n.push_back(rhs.getWeight());
    n.push_back(rhs.getWeightSQ());
  ...
  static bool decode(const Node &n,
                     cslibs_math::statistics::StableWeightedDistribution<
                         T, 1, lambda_ratio_exponent> &rhs) {
    if (!n.IsSequence() || n.size() != 5) return false;

    rhs = cslibs_math::statistics::StableWeightedDistribution<
        T, 1, lambda_ratio_exponent>(n[0].as<std::size_t>(), n[1].as<T>(),
                                     n[2].as<T>(), n[3].as<T>(), n[4].as<T>());
    return true;
  }
};
}  // namespace YAML
