...
struct binary<cslibs_math::statistics::WeightedDistribution, T, Dim,
              lambda_ratio_exponent> {
  using distribution_t =
      cslibs_math::statistics::WeightedDistribution<T, Dim,
                                                    lambda_ratio_exponent>;
...
namespace YAML {
template <typename T, std::size_t Dim, std::size_t lambda_ratio_exponent>
struct convert<cslibs_math::statistics::WeightedDistribution<
    T, Dim, lambda_ratio_exponent>> {
  using sample_t = typename cslibs_math::statistics::WeightedDistribution<
      T, Dim, lambda_ratio_exponent>::sample_t;
  using covariance_t = typename cslibs_math::statistics::WeightedDistribution<
      T, Dim, lambda_ratio_exponent>::covariance_t;

  static Node encode(const cslibs_math::statistics::WeightedDistribution<
  ...
  static bool decode(const Node &n,
                     cslibs_math::statistics::WeightedDistribution<
                         T, Dim, lambda_ratio_exponent> &rhs) {
    ...
    rhs = cslibs_math::statistics::WeightedDistribution<T, Dim,
                                                        lambda_ratio_exponent>(
        sc, w, w_sq, mean, correlated);
    return true;
  }
};
...
struct convert<cslibs_math::statistics::WeightedDistribution<
  ...
  static bool decode(
      const Node &n,
      cslibs_math::statistics::WeightedDistribution<T, 1, lambda_ratio_exponent>
          &rhs) {
    if (!n.IsSequence() || n.size() != 5) return false;

    rhs = cslibs_math::statistics::WeightedDistribution<T, 1,
                                                        lambda_ratio_exponent>(
        n[0].as<std::size_t>(), n[1].as<T>(), n[2].as<T>(), n[3].as<T>(),
        n[4].as<T>());
    return true;
  }
};
