...
struct Index : public std::array<int, _Nm> {
   ...
   */
  explicit Index(const int i = 0) { base_t::fill(i); }

  /**
   * @brief Index constructor by std::arra<int, Dim>.
   * @param other - the other index
   */
  explicit Index(const std::array<int, _Nm> &arr) : std::array<int, _Nm>{arr} {}

  void operator=(const int i) { base_t::fill(i); }
...
template <std::size_t _Nm>
inline cslibs_math::common::Index<_Nm> operator*(
    const cslibs_math::common::Index<_Nm> &__arr, const int __s) {
  cslibs_math::common::Index<_Nm> __res;
  for (std::size_t i = 0; i < _Nm; ++i) __res[i] = __arr[i] * __s;
  return __res;
}

template <std::size_t _Nm>
inline cslibs_math::common::Index<_Nm> operator-(
    const cslibs_math::common::Index<_Nm> &__one,
    const cslibs_math::common::Index<_Nm> &__two) {
  cslibs_math::common::Index<_Nm> __res;
  for (std::size_t i = 0; i < _Nm; ++i) __res[i] = __one[i] - __two[i];
...
template <std::size_t _Nm>
inline cslibs_math::common::Index<_Nm> operator-(
    const cslibs_math::common::Index<_Nm> &__idx, const int __scalar) {
  cslibs_math::common::Index<_Nm> __res;
  for (std::size_t i = 0; i < _Nm; ++i) __res[i] = __idx[i] - __scalar;
...
inline cslibs_math::common::Index<_Nm> operator<(
    ...
    const cslibs_math::common::Index<_Nm> &__two) {
  bool less = true;
  for (std::size_t i = 0; i < _Nm; ++i) less &= __one[i] < __two[i];
  return less;
}
...
inline cslibs_math::common::Index<_Nm> operator>(
    ...
    const cslibs_math::common::Index<_Nm> &__two) {
  bool greater = true;
  for (std::size_t i = 0; i < _Nm; ++i) greater &= __one[i] > __two[i];
  return greater;
}
...
inline cslibs_math::common::Index<_Nm> operator<=(
  ...
  for (std::size_t i = 0; i < _Nm; ++i) less_equal &= __one[i] <= __two[i];
  return less_equal;
}
...
inline cslibs_math::common::Index<_Nm> operator>=(
  ...
  for (std::size_t i = 0; i < _Nm; ++i) greater_equal &= __one[i] >= __two[i];
