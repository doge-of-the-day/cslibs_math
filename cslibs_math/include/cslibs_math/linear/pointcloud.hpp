...
class EIGEN_ALIGN16 Pointcloud {
 public:
  ...
  inline Pointcloud() = default;
  inline Pointcloud(const Pointcloud &other) = default;
  inline Pointcloud(Pointcloud &&other) = default;
  ...
  inline virtual void insertInvalid() { data_.emplace_back(point_t::inf()); }

  inline virtual void clear() { data_.clear(); }

  inline const_iterator_t begin() const { return data_.begin(); }

  inline const_iterator_t end() const { return data_.end(); }

  inline point_t const &at(const std::size_t i) const { return data_.at(i); }

  inline points_t const &getPoints() const { return data_; }

  inline void reserve(const std::size_t s) { return data_.reserve(s); }

  inline std::size_t size() const { return data_.size(); }

  inline const point_t &min() const { return min_; }

  inline const point_t &max() const { return max_; }
  ...
  inline void transform(const transform_t &t) {
    ...
    min_ = point_t::max();
    max_ = point_t::min();
    std::for_each(data_.begin(), data_.end(), apply);
  }

 protected:
  points_t data_;
  point_t min_{point_t::max()};
  point_t max_{point_t::min()};
};
