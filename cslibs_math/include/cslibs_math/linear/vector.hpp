...
class EIGEN_ALIGN16 Vector {
 public:
  ...
  inline explicit Vector(const T &c) : data_{vector_t::Constant(c)} {}

  inline explicit Vector(const arr_t &a) : data_{a.data()} {}

  inline explicit Vector(vector_t &&data) : data_{std::move(data)} {}

  inline explicit Vector(const vector_t &data) : data_{data} {}

  inline Vector(const Vector &other) = default;
  inline Vector(Vector &&other) = default;

  inline Vector(const Matrix<T, Dim, 1> &m) : data_{m.data()} {}

  template <typename... args_t,
            typename = typename std::enable_if<sizeof...(args_t) == Dim>>
  inline explicit Vector(const args_t... values)
      : data_{eigen::create<vector_t>(values...)} {
    static_assert(
        Dimension == sizeof...(args_t),
        "Dimension of the parameters must match the geometric dimension!");
  }
  ...
  inline T dot(const Vector &other) const { return data_.dot(other.data_); }

  inline Vector cross(const Vector &other) {
    return Vector(data_.cross(other.data_));
  }

  inline T length() const { return data_.norm(); }

  inline T length2() const { return data_.squaredNorm(); }

  inline T operator()(const std::size_t i) const { return data_(i); }

  inline T &operator()(const std::size_t i) { return data_(i); }
  ...
  inline Vector operator-() const { return Vector(vector_t(-data_)); }

  inline Vector &min(const Vector &other) {
    data_ = (data_.cwiseMin(other.data_)).eval();
    return *this;
  }

  inline Vector &max(const Vector &other) {
    data_ = (data_.cwiseMax(other.data_)).eval();
    return *this;
  }

  operator vector_t const &() const { return data_; }

  operator vector_t &() { return data_; }

  operator vector_t() const { return data_; }

  operator vector_t *() { return &data_; }

  inline const vector_t &data() const { return data_; }

  inline vector_t &data() { return data_; }

  inline bool isNormal() const { return eigen::isnormal(data_); }
  ...
  inline static Vector max() { return Vector(std::numeric_limits<T>::max()); }
...
template <typename T, std::size_t Dim>
inline cslibs_math::linear::Vector<T, Dim> min(
    const cslibs_math::linear::Vector<T, Dim> &a,
    const cslibs_math::linear::Vector<T, Dim> &b) {
  using vector_t = typename cslibs_math::linear::Vector<T, Dim>::vector_t;
  return cslibs_math::linear::Vector<T, Dim>(
      vector_t(a.data().cwiseMin(b.data())));
}

template <typename T, std::size_t Dim>
inline cslibs_math::linear::Vector<T, Dim> max(
    const cslibs_math::linear::Vector<T, Dim> &a,
    const cslibs_math::linear::Vector<T, Dim> &b) {
  using vector_t = typename cslibs_math::linear::Vector<T, Dim>::vector_t;
  return cslibs_math::linear::Vector<T, Dim>(
      vector_t(a.data().cwiseMax(b.data())));
}
}  // namespace linear
}  // namespace cslibs_math

template <typename T, std::size_t Dim>
inline cslibs_math::linear::Vector<T, Dim> operator*(
    const cslibs_math::linear::Vector<T, Dim> &v, const T s) {
  using vector_t = typename cslibs_math::linear::Vector<T, Dim>::vector_t;
