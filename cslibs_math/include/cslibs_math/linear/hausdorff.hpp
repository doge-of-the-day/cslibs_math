...
template <typename T = double, typename point_t>
inline std::size_t nearestNeighbour(
    const point_t &point,
    const typename cslibs_math::linear::Pointcloud<point_t> &points) {
...
template <typename T = double, typename point_t>
inline T hausdorff(
    const typename cslibs_math::linear::Pointcloud<point_t> &points_src,
...
template <typename T = double, typename point_t>
inline T hausdorffFraction(
    const typename cslibs_math::linear::Pointcloud<point_t> &points_src,
    const typename cslibs_math::linear::Pointcloud<point_t> &points_dst,
    const T max_dist) {
  if (points_src.size() == 0) return 0.0;
...
template <typename T = double, typename point_t>
inline T hausdorffAvg(
    const typename cslibs_math::linear::Pointcloud<point_t> &points_src,
    const typename cslibs_math::linear::Pointcloud<point_t> &points_dst) {
  if (points_src.size() == 0) return std::numeric_limits<T>::infinity();
...
template <typename T = double, typename point_t>
inline Matrix<T, point_t::Dimension, point_t::Dimension> hausdorffCovariance(
    const typename cslibs_math::linear::Pointcloud<point_t> &points_src,
    const typename cslibs_math::linear::Pointcloud<point_t> &points_dst) {
  ...
  for (const point_t &point_src : points_src) {
    if (point_src.isNormal()) {
      ...
      if (nn == std::numeric_limits<std::size_t>::infinity()) continue;
