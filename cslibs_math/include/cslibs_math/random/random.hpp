...
/**
 * @brief Interface for random generator independent from dimensionality and
 * distribution type.
 */
...
class EIGEN_ALIGN16 Uniform : public RandomGenerator<Generator> {
 public:
  ...
  inline void set(const sample_t &min, const sample_t &max) {
...
class EIGEN_ALIGN16 Uniform<T, 1, Generator>
 ...
 public:
  ...
  inline T get() { return distribution_(base_t::random_engine_); }
  ...
  inline void get(T &sample) { sample = distribution_(base_t::random_engine_); }
...
class EIGEN_ALIGN16 Normal : public RandomGenerator<Generator> {
 public:
  ...
  inline void set(const sample_t &mean, const matrix_t &covariance) {
    mean_ = mean;
    covariance_ = covariance;

    solver_t eigen(covariance_);
    rotation_ =
        eigen.eigenvectors().real();  /// rotation into the "world_frame"
    scale_ = eigen.eigenvalues()
                 .real()
                 .cwiseSqrt();  /// scale along the main axis of distribution
  }
 ...
 */
template <typename T, typename Generator>
class EIGEN_ALIGN16 Normal<T, 1, Generator>
    : public RandomGenerator<Generator> {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  inline void set(const T mean, const T _sigma) {
