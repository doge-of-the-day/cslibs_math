...
template <typename T>
using boost_line_t =
    typename cslibs_boost_geometry::types::Line<boost_point_t<T>>::type;
template <typename T>
using boost_pointset_t =
    typename cslibs_boost_geometry::types::PointSet<boost_point_t<T>>::type;
template <typename T>
using boost_lineset_t =
    typename cslibs_boost_geometry::types::LineSet<boost_point_t<T>>::type;
...
template <typename T>
inline void from(const boost_pointset_t<T> &src, std::vector<Point2<T>> &dst) {
  dst.resize(src.size());
...
template <typename T>
inline void from(const boost_lineset_t<T> &src, std::vector<Line2<T>> &dst) {
  dst.resize(src.size());
  std::transform(src.begin(), src.end(), dst.begin(),
...
template <typename T>
inline void from(const std::vector<Point2<T>> &src, boost_pointset_t<T> &dst) {
  dst.resize(src.size());
  std::transform(src.begin(), src.end(), dst.begin(),
...
template <typename T>
inline void from(const std::vector<Line2<T>> &src, boost_lineset_t<T> &dst) {
  dst.resize(src.size());
  std::transform(src.begin(), src.end(), dst.begin(),
