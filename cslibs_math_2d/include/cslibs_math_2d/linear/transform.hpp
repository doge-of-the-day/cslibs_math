...
template <typename T>
inline cslibs_math_2d::Vector2<T> operator*(
    const cslibs_math_2d::Transform2<T> &t,
...
class EIGEN_ALIGN16 Transform2 {
 public:
  ...
  inline explicit Transform2(const T x, const T y) : translation_{x, y} {}
  ...
  inline static Transform2 identity() { return Transform2(); }
  ...
  inline Transform2 inverse() const {
    return Transform2(
        Vector2<T>(-(cos_ * translation_(0) + sin_ * translation_(1)),
                   -(-sin_ * translation_(0) + cos_ * translation_(1))),
        -yaw_, -sin_, cos_);
  }

  inline Transform2 operator-() const { return inverse(); }
  ...
  inline Vector2<T> &translation() { return translation_; }

  inline Vector2<T> const &translation() const { return translation_; }
  ...
  inline void setFrom(const T x, const T y, const T yaw) {
    translation_(0) = x;
    translation_(1) = y;
    setYaw(yaw);
  }

  inline Transform2 interpolate(const Transform2 &other, const T ratio) const {
    assert(ratio >= 0.0);
    assert(ratio <= 1.0);
    if (ratio == T()) return *this;

    if (ratio == T(1.0)) return other;

    const T ratio_inverse = T(1.0) - ratio;
    const Vector2<T> translation =
        translation_ * ratio_inverse + other.translation_ * ratio;
    const T yaw = cslibs_math::common::angle::normalize(
        yaw_ +
        ratio * cslibs_math::common::angle::normalize(other.yaw_ - yaw_));
 ...
 private:
  Vector2<T> translation_{Vector2<T>(T{0}, T{0})};
  T yaw_{T{0}};
  T sin_{T{0}};
  T cos_{T{1}};
};
...
inline cslibs_math_2d::Vector2<T> operator*(
  ...
  return t.yaw() == T()
             ? v + t.translation()
             : cslibs_math_2d::Vector2<T>(
                   t.cos() * v(0) - t.sin() * v(1) + t.translation()(0),
                   t.sin() * v(0) + t.cos() * v(1) + t.translation()(1));
}

template <typename T>
inline cslibs_math_2d::Transform2<T> operator*(
    const cslibs_math_2d::Transform2<T> &a,
    const cslibs_math_2d::Transform2<T> &b) {
  return a.yaw() == T()
             ? cslibs_math_2d::Transform2<T>(b.translation() + a.translation(),
                                             b.yaw(), b.sin(), b.cos())
             : b.yaw() == T()
                   ? cslibs_math_2d::Transform2<T>(a * b.translation(), a.yaw(),
                                                   a.sin(), a.cos())
                   : cslibs_math_2d::Transform2<T>(
                         a * b.translation(),
                         cslibs_math::common::angle::normalize(a.yaw() +
                                                               b.yaw()),
