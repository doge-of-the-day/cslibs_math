...
class EIGEN_ALIGN16 PolarPoint2 {
 public:
  ...
  inline PolarPoint2() = default;

  inline explicit PolarPoint2(const T theta, const T rho)
      : cartesian_{std::cos(theta) * rho, std::sin(theta) * rho},
        theta_{theta},
        rho_{rho} {}

  inline explicit PolarPoint2(const point_t &cartesian)
      : cartesian_{cartesian},
        theta_{cslibs_math_2d::angle(cartesian)},
        rho_{cartesian.length()} {}

  inline explicit PolarPoint2(const point_t &cartesian, const T theta, const T rho)
      : cartesian_{cartesian}, theta_{theta}, rho_{rho} {}

  inline PolarPoint2(const PolarPoint2 &other) = default;
  inline PolarPoint2(PolarPoint2 &&other) = default;
  ...
  inline T getRho() const { return rho_; }

  inline T getTheta() const { return theta_; }
  ...
  inline point_t const &getCartesian() const { return cartesian_; }
 ...
 private:
  point_t cartesian_{std::numeric_limits<T>::infinity()};
  T theta_{0};
  T rho_{std::numeric_limits<T>::infinity()};
