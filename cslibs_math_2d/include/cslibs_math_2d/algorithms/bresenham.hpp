...
class EIGEN_ALIGN16 Bresenham {
 public:
  ...
  template <typename T>
  inline explicit Bresenham(const Point2<T> &p0, const Point2<T> &p1,
                            const T resolution)
      : Bresenham(
            {{static_cast<int>(std::floor(p0(0) / resolution)),
  ...
  inline int x() const { return steep_ ? index_[1] : index_[0]; }

  inline int y() const { return steep_ ? index_[0] : index_[1]; }

  inline index_t operator()() const { return {{x(), y()}}; }

  inline Bresenham &operator++() { return done() ? *this : iterate(); }
