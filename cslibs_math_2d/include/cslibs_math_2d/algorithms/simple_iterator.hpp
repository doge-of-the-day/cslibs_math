#ifndef CSLIBS_MATH_2D_SIMPLE_ITERATOR_HPP
#define CSLIBS_MATH_2D_SIMPLE_ITERATOR_HPP

#include <cslibs_math/common/array.hpp>
#include <cslibs_math/common/floor.hpp>
#include <cslibs_math_2d/linear/point.hpp>
#include <memory>

namespace cslibs_math_2d {
namespace algorithms {
template <typename T>
class EIGEN_ALIGN16 SimpleIterator {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  using Ptr = std::shared_ptr<SimpleIterator<T>>;

  using index_t = std::array<int, 2>;
  using point_t = Point2<T>;

  inline explicit SimpleIterator(const index_t &start, const index_t &end)
      : SimpleIterator{point_t{start[0], start[1]}, point_t{end[0], end[1]},
                       1.0} {}

  inline explicit SimpleIterator(const point_t &p0, const point_t &p1,
                                 const T &resolution)
      : index_{static_cast<int>(std::floor(p0(0) / resolution)),
               static_cast<int>(std::floor(p0(1) / resolution))},
        end_{static_cast<int>(std::floor(p1(0) / resolution)),
             static_cast<int>(std::floor(p1(1) / resolution))},
        resolution_inv_{1.0 / resolution},
        diff_{p1 - p0},
        point_{p0} {
    N_ = static_cast<int>(diff_.length() * resolution_inv_);
    diff_ /= static_cast<T>(N_);
  }

  inline int x() const { return index_[0]; }

  inline int y() const { return index_[1]; }

  inline index_t operator()() const { return index_; }

  inline SimpleIterator &operator++() {
    if (done()) return *this;

    index_t i = index_;
    do {
      point_ += diff_;
      --N_;
      i[0] = cslibs_math::common::floor(point_(0) * resolution_inv_);
      i[1] = cslibs_math::common::floor(point_(1) * resolution_inv_);
    } while (i == index_);
    index_ = i;

    return *this;
  }

  inline bool done() const {
    return (index_[0] == end_[0] && index_[1] == end_[1]) || N_ < 0;
  }

 private:
  index_t index_;
  index_t end_;
  T resolution_inv_;
  point_t diff_;
  point_t point_;
  int N_;
};
}  // namespace algorithms
}  // namespace cslibs_math_2d

#endif  // CSLIBS_MATH_2D_SIMPLE_ITERATOR_HPP
